...
- **ViewModel (Hook)**: Implemented as a React Hook (`useTimelineViewModel.js`).
...
- **View (Presentational Component)**: Implemented as a React component (`TimelineView.jsx`).
  - It is a stateless component that receives the `timelineName` and the processed `events` array from the ViewModel.
  - Its sole responsibility is to render the UI based on the props it receives.
- **Main Component**: The `Timeline.jsx` component composes the ViewModel and the View, serving as the public-facing entry point for the feature.
